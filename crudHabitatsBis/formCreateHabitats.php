...
if (isset($_POST["submit"])) {
    ...
    $description_habitat = htmlspecialchars($_POST['description_habitat'] ?? '');
    $prenom_animal = htmlspecialchars($_POST['prenom_animal'] ?? '');
    $race = htmlspecialchars($_POST['race'] ?? '');
    $etat_sante = htmlspecialchars($_POST['etat_sante'] ?? '');
    $detail_sante = htmlspecialchars($_POST['detail_sante'] ?? '');
    ...
    if (isset($_FILES['img'])) {
        foreach ($_FILES['img']['name'] as $key => $image) {
            ...
            if ($_FILES['img']['error'][$key] === UPLOAD_ERR_OK && move_uploaded_file($_FILES['img']['tmp_name'][$key], $targetFile)) {
                $uploadedImages[] = $targetFile;
            } else {
                $errorMessage .= "Erreur lors du téléchargement de l'image " . htmlspecialchars($_FILES['img']['name'][$key]) . ". ";
            }
        }
    }
    ...
    try {
        ...
        $sql = "INSERT INTO habitatsBis (habitat, description_habitat, img, prenom_animal, race, etat_sante, detail_sante, nourriture, grammage, date_visite, date_repas_pris, heure_repas_pris) 
        ...
        if ($query->execute()) {
            ...
            $errorMessage .= "Erreur d'insertion du rapport : " . implode(", ", $query->errorInfo());
        }
    } catch (PDOException $e) {
        $errorMessage .= "Erreur lors de l'insertion du rapport : " . $e->getMessage();
    }
}
?>
