...
$sql = "SELECT h.*, hi.image 
        FROM habitatsBis h 
        LEFT JOIN habitat_imageBis hi ON h.id_habitat = hi.habitat_id";
...
while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
    ...
    if (!isset($habitatsBis[$id_habitat])) {
        $habitatsBis[$id_habitat] = [
            'id_habitat' => $row['id_habitat'],
            'habitat' => $row['habitat'],
    ...
    if ($row['image']) {
        $habitatsBis[$id_habitat]['images'][] = $row['image'];
    }
}
?>
    ...
    <section class="container my-5 mt-4">
        ...
        <table class="table" data-toggle="table" data-search="true" data-show-columns="true" data-pagination="true">
            ...
            <tbody>
                ...
                foreach ($habitatsBis as $habitat) {
                    $imgTags = '';
